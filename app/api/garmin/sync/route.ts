...
import { enrichStravaActivitiesWithGarmin, establishGarminClient, friendlyGarminError, syncGarminHealthEntryForToday } from "@/lib/garmin";
...
export async function POST() {
  ...
  let tokens;
  try {
    ({ client, tokens } = await establishGarminClient(connection));
  } catch (err) {
