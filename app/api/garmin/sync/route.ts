...
import { friendlyGarminError, garminLogin, garminRestoreSession, syncGarminHealthEntryForToday, type GarminTokens } from "@/lib/garmin";
...
export async function POST() {
  ...
  try {
    ...
    await supabase
      ...
      .eq("user_id", user.id);
    return NextResponse.json({ synced: true, syncedFields });
  } catch (err) {
